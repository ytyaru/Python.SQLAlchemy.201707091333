...
s = sqlalchemy.sql.select([db.Tables['Addresses']]).group_by(db.Tables['Addresses'].c.MailAddress)
print(s)
for row in conn.execute(s): print(row)

s = sqlalchemy.sql.select([db.Tables['Addresses']]).group_by(db.Tables['Addresses'].c.MailAddress).having(15 <= db.Tables['Addresses'].c.Id)
print(s)
for row in conn.execute(s): print(row)
