...
#s = sqlalchemy.sql.select([sqlalchemy.sql.table('Users')])
s = sqlalchemy.sql.select([db.Tables['Users']]).order_by(sqlalchemy.sql.expression.desc('Id'))
print(s)
for row in conn.execute(s): print(row)

#print(sqlalchemy.sql.table('Users'))
#print(sqlalchemy.sql.table('Addresses'))
#print(sqlalchemy.sql.table)
#for t in sqlalchemy.sql.table(): print(t)
